...
## v2.0 — 2026-07-30(系列第 6 篇:开会打哈欠传染, 其实是厌倦传染)

生活小事角度池第三篇, 候选 #7 触发。跟 004 配对(都落"过程 > 结论", 切口不同)。

### 1. 006 主题与结构

- **主题**:生活小事 — 过程 > 结论(候选 #7 触发, 跟 004 配对)
- **标题**:开会打哈欠传染, 其实是厌倦传染
- **digest**:哈欠传染的物理机制是镜像神经元。但会议上, 1 小时后整桌都打, 不是传染, 是大家自己已经累了。哈欠是结论, 厌倦是过程。
- **结构**:引子(11 个人都打哈欠) → 段 1(哈欠传染的物理机制) → 段 2(会议上的哈欠不一样, 1 小时后才开始) → 段 3(哈欠是结论, 厌倦是过程) → 段 4(开会不再忍哈欠, 跟 005 撤回是反的) → 段 5(提醒我们的不是镜像神经元, 是会议本身) → 收束引用块
- **字数**:~ 800 字
- **暗合核心三句话**:"过程 > 结论"(哈欠是结论/厌倦是过程);"草稿 > 作品"(跟 005 联动, 撤回=抹掉草稿, 忍哈欠=改过程为结论)
- **同 004/005 配对**:004 掏出手机 vs 30 秒发呆, 005 撤回 vs 改错字, 006 哈欠 vs 厌倦 — 三个不同切口都讲"过程/草稿 价值"

### 2. v1.8 IP 决策规律第五次应用

006 的 4 张图(1 cover + 3 正文):

| 图 | 动作/状态 | 决策 | 新文件名 |
|---|---|---|---|
| 00 cover | 状态: 会议桌 + 哈欠 + 1 小时 | 去 IP | `00-meeting.png` |
| 01 看别人打 | 动作: 看到同事打哈欠 | 保留 IP | `01-saw-yawn.png` |
| 02 时间轴 | 状态: 1 小时 → 11 人打 | 去 IP | `02-timeline.png` |
| 03 自己打哈欠 | 动作: 闭眼 + 哈欠 | 保留 IP | `03-self-yawn.png` |

2 张 IP(动作型:看/打) + 2 张去 IP(状态型:会议桌/时间轴), 跟 v1.8 决策一致

### 3. 三维轮换更新

- **表情**:1 default + 1 thoughtful + 1 default(2 种, 跟 005 一致)
- **姿态**:新姿态:坐会议桌前+看到同事打哈欠 / 闭眼打哈欠+手撑下巴
- **物件**:新物件:会议桌 / 椅子 / 笔记本(没开) / 时钟(挂在墙上) / 哈欠符号 / 同事剪影
- **装扮**:0/3 用, 跟 004/005 一致

### 4. 文件变更(7 个)

- `doc/essays/006-yawn-contagion.md`(新)
- `doc/essays/images/006-yawn-contagion/00-meeting.png`(新, 去 IP)
- `doc/essays/images/006-yawn-contagion/01-saw-yawn.png`(新, IP deadpan)
- `doc/essays/images/006-yawn-contagion/02-timeline.png`(新, 去 IP)
- `doc/essays/images/006-yawn-contagion/03-self-yawn.png`(新, IP thoughtful)
- `xiaofan-ink/assets/prompts/articles/006-yawn-contagion.md`(新, 实战 prompt 文档)
- `doc/SERIES-STATE.md`(加 006 行 + 006 跟 004 配对说明 + 已用姿态/物件更新)
- `doc/essays/README.md`(加 006 + 系列三联说明)
- `README.md`(根, 目录树加 006)
- `doc/CREATION-PLAN.md`(演化日志加 v2.0 entry for 006)
- `CHANGELOG.md`(本条)

### 5. 候选池(5 个未写, 等触发)

- 候选 1:超市试吃装的不是样品, 是剧本
- 候选 3:出门忘带钥匙, 10 年仪式 1 次破功
- 候选 5:PPT 改 5 版, 改稿改的是起点的灵气
- 候选 6:便利店饭团, 最顺手的不是最对的
- 候选 8:退订公众号, 关注不是喜欢, 是不敢说不

### 6. 用户后续

- 公众号后台 → 草稿箱 → 留 006 这次的新草稿
- 不催任何候选, 等触发

---

## v2.0 — 2026-07-29(系列第 5 篇:微信撤回消息, 撤回不是修改, 是暴露)

生活小事角度池第二篇, 候选 #4 触发。跟 004 "过程 > 结论" 配对, 005 落"草稿 > 作品"。

### 1. 005 主题与结构

- **主题**:生活小事 — 草稿 > 作品(候选 #4 触发)
- **标题**:微信撤回消息, 撤回不是修改, 是暴露
- **digest**:撤回是 2 分钟内反悔, 但反悔这件事所有人都看见。撤回不是把草稿改成作品, 是把"那个草稿的我" 抹掉。
- **结构**:引子(我做过很多次撤回) → 段 1(撤回的两个规则) → 段 2(撤回不是修改) → 段 3(把"草稿的我" 抹掉了) → 段 4(我现在不太撤回了) → 段 5(留个缝) → 收束引用块
- **字数**:~ 800 字
- **暗合核心三句话**:"草稿 > 作品"(撤回破坏"草稿的我"); "死板冷静"(对工具/社交礼仪的冷静反思)

### 2. v1.8 IP 决策规律第四次应用

005 的 4 张图(1 cover + 3 正文):

| 图 | 动作/状态 | 决策 | 新文件名 |
|---|---|---|---|
| 00 cover | 状态: 微信界面 + 撤回灰字 | 去 IP | `00-recall-prompt.png` |
| 01 看手机 | 动作: 看到撤回提示 | 保留 IP | `01-saw-recall.png` |
| 02 时间轴 | 状态: 草稿 → 撤回对比 | 去 IP | `02-timeline.png` |
| 03 想 | 动作: 想那条消息是啥 | 保留 IP | `03-thinking.png` |

2 张 IP(动作型:看/想) + 2 张去 IP(状态型:微信界面/时间轴), 跟 v1.8 配图去 IP 化决策一致

### 3. 三维轮换更新

- **表情**:跟 004 一样, 1 default + 1 thoughtful + 1 default(收束 03 复用 default, 不算新)
- **姿态**:新姿态:站+看手机 / 拿手机看向上方
- **物件**:新物件:手机(大屏特写) / 微信对话框 / "X 撤回了一条消息" 灰字 / 撤回 2 分钟倒计时
- **装扮**:0/3 用, 跟 004 一致

### 4. 文件变更(7 个)

- `doc/essays/005-wechat-recall.md`(新)
- `doc/essays/images/005-wechat-recall/00-recall-prompt.png`(新, 去 IP)
- `doc/essays/images/005-wechat-recall/01-saw-recall.png`(新, IP deadpan)
- `doc/essays/images/005-wechat-recall/02-timeline.png`(新, 去 IP)
- `doc/essays/images/005-wechat-recall/03-thinking.png`(新, IP thoughtful)
- `xiaofan-ink/assets/prompts/articles/005-wechat-recall.md`(新, 实战 prompt 文档)
- `doc/SERIES-STATE.md`(加 005 行 + 更新已用姿态/物件 + 005 跟 004 配对说明)
- `doc/CREATION-PLAN.md`(演化日志加 v2.0 entry for 005)
- `CHANGELOG.md`(本条 v2.0 for 005)

### 5. 用户后续

- 公众号后台 → 草稿箱 → 这次 005 没旧草稿(没生成过) → 留 v2.0 这次的 005 新草稿
- 跟 000/001/002/003/004 节奏一致:想写就写, 不定时间
- **不催 #7(打哈欠传染)** 也不催 #1/3/6/8 —— 等用户触发信号再写

---

## v2.0 — 2026-07-27(系列新阶段:工具三部曲收尾 + 生活小事角度池开篇)

系列从"工具反思" 单角度池扩到"工具反思 + 生活小事" 双角度池。**核心三句话不变**(过程 > 结论 / 草稿 > 作品 / 死板冷静),新角度池只是开新支线,不挤占原有角度。

### 1. 角度池扩展(关键决策)

- **背景**:001/002/003 形成"工具三部曲"(节奏/专注/控制权)收尾,后续工具反思类会角度枯竭
- **新方向**:生活小事(等红灯/打哈欠/试吃等日常小场景)切入,落到核心三句话之一
- **调性保障**:每篇仍是 真发生过的具体小事 + 反共识道理 + 短句 + 冷幽默收尾
- **避坑**:不卖萌 / 不温暖治愈 / 不积极向上
- **候选池**:7 个候选(等红灯/打哈欠/试吃/出门忘钥匙/微信撤回/PPT 改 5 版/便利店饭团/退订公众号)

### 2. 004 主题与结构

- **主题**:生活小事 — 过程 > 结论(新角度池开篇)
- **标题**:等红灯 30 秒, 我以为我需要信息, 其实我需要 30 秒
- **digest**:红灯 30 秒, 一直掏手机。1 次没掏, 30 秒发呆, 想到一个之前没想到的小事。掏出手机拿的是结论, 30 秒发呆拿的是过程。
- **结构**:引子(5 年掏手机习惯) → 段 1(1 次没掏, 站了 30 秒) → 段 2(30 秒里想到 3 周前发的消息) → 段 3(掏出手机 = 拿结论, 30 秒发呆 = 拿过程) → 段 4(跟"草稿 vs 作品"一回事) → 收束引用块
- **字数**:~ 750 字
- **暗合核心三句话**:"过程 > 结论"(掏出手机拿信息=拿结论,30 秒发呆=拿过程);"草稿 > 作品"(刷到的=别人作品,想到的=自己草稿)

### 3. v1.8 IP 决策规律第三次应用

004 的 4 张图(1 cover + 3 正文):

| 图 | 动作/状态 | 决策 | 新文件名 |
|---|---|---|---|
| 00 cover | 状态: 路口 + 没人 | 去 IP | `00-red-light.png` |
| 01 站等红灯 | 动作: 掏到一半 | 保留 IP | `01-standing.png` |
| 02 站发愣 | 动作: 灯变绿了 | 保留 IP | `02-lights-green.png` |
| 03 走过路口 | 动作: 慢走 | 保留 IP | `03-crossed.png` |

3 张 IP(动作型) + 1 张去 IP(状态型:城市路口), 跟 v1.8 配图去 IP 化决策一致

### 4. 三维轮换更新

- **表情**:首次把"思考"(07-thoughtful) 加入"已用"池(原 1 default + 1 confused + 1 satisfied, 现 1 default + 1 thoughtful + 1 satisfied)
- **姿态**:新姿态:站 + 拿手机没看 / 站 + 发愣 / 慢走
- **物件**:新物件:路口 / 红灯 / 手机(没看) / 行人信号灯 / 朋友对话框(文本)
- **装扮**:故意 0/3 用, 强调"我 vs 信息" 的人物关系

### 5. 流程不变,但加用户参与环节

- **v1.2-v1.9 流程**:agent 自主从候选池选 → 用户审稿 → 用户决定发布
- **v2.0 起流程(004)**:agent 提候选池(7 个) → 用户挑(选了 #2) → agent 写文 + 出图 → 用户审 → commit + push + 同步公众号
- **理由**:生活小事角度池候选需要"对味儿"——只有用户自己最清楚哪个小事戳自己

### 6. 候选池锁定(7 个, 等触发信号)

- 候选 2:等红灯 30 秒(本次写完)✅
- 候选 7:开会打哈欠传染(已敲定, 等用户触发再写)
- 候选 1:超市试吃装的不是样品, 是剧本
- 候选 3:出门忘带钥匙, 10 年仪式 1 次破功
- 候选 4:微信撤回消息, 撤回不是修改, 是暴露
- 候选 5:PPT 改 5 版, 改稿改的是起点的灵气
- 候选 6:便利店饭团, 最顺手的不是最对的
- 候选 8:退订公众号, 关注不是喜欢, 是不敢说不
...
- `doc/essays/004-red-light-30-seconds.md`(新)
- `doc/essays/images/004-red-light-30-seconds/00-red-light.png`(新, 去 IP)
- `doc/essays/images/004-red-light-30-seconds/01-standing.png`(新, IP deadpan)
- `doc/essays/images/004-red-light-30-seconds/02-lights-green.png`(新, IP thoughtful)
- `doc/essays/images/004-red-light-30-seconds/03-crossed.png`(新, IP satisfied)
- `xiaofan-ink/assets/prompts/articles/004-red-light-30-seconds.md`(新, 实战 prompt 文档)
- `doc/SERIES-STATE.md`(加 004 行 + 加"角度池"段 + 加"生活小事角度池规则")
- `doc/CREATION-PLAN.md`(演化日志加 v2.0 entry)
- `CHANGELOG.md`(本条 v2.0)

### 8. 用户后续

- 公众号后台 → 草稿箱 → 这次 004 没旧草稿(没生成过) → 留 v2.0 这次的 004 新草稿
- 跟 000/001/002/003 节奏一致:想写就写, 不定时间
- **不催 #7(打哈欠传染)**:等用户触发信号再写, 跟"想写就写"调性自洽

---

## v1.9 — 2026-07-22(系列第 3 篇:Claude Code 用了一周, 我关掉了)

跟 001 / 002 配对成"工具三部曲"收尾:001 谈节奏(每日计划),002 谈专注(番茄钟),**003 谈控制权(AI 编程工具依赖感)**。

切口跟前两篇不重复:不是"AI 编程工具有多强"或"AI 替代论",是"用了之后我失去了什么" — 跟 000 的"过程 > 结论 / 草稿 > 作品"核心三句话直接呼应(AI 跳过草稿直接交作品, 失去的是过程)。

### 1. 003 主题与结构

- **主题**:工具反思 — 控制权(跟 001 节奏 / 002 专注配对, 工具三部曲收尾)
- **切口**:对 AI 编程工具的依赖感(2026 年 Claude Code / Cursor / Codex 全面爆发, "AI 依赖反思" 是技术圈最热话题之一)
- **标题**:Claude Code 用了一周, 我关掉了(同 001/002 结构:时段 + 我做了什么 + 反转)
- **digest**:它写得比 80% 的同事都干净。第三天, 我发现我已经不认识自己的代码。过程没了, 草稿没了。
- **结构**:引子 → 段 1(第三天慌) → 段 2(不是 AI 写得不好) → 段 3(草稿跟作品的差别) → 段 4(现在怎么用) → 收束引用
- **字数**:~ 750 字
- **同系列呼应**:跟 000 "过程 > 结论" 核心三句话直接呼应;跟 002 "工具站错位" 反思路径同构(番茄钟站"开始专注"位错, Claude Code 站"替你写"位也错)

### 2. v1.8 IP 决策规律二次应用

003 的 5 张图(1 cover + 4 正文)应用 v1.8 决策表:

| 图 | 动作/状态 | 决策 | 新文件名 |
|---|---|---|---|
| 00 cover | 状态: 陌生人代码 | 去 IP | `00-stranger-code.png` |
| 01 看终端 | 动作: 站+看代码滚 | 保留 IP | `01-staring-terminal.png` |
| 02 散落代码片段 | 状态: 散乱 | 去 IP | `02-stray-snippets.png` |
| 03 切回敲键盘 | 动作: 切回手动 | 保留 IP | `03-typing-back.png` |
| 04 站桌回看 | 动作: 站+回看 | 保留 IP | `04-looking-back.png` |

3 张 IP(动作型:看/敲/回看)+ 2 张去 IP(状态型:陌生人代码/散落) — 跟 v1.8 配图去 IP 化决策一致

### 3. 三维轮换更新

- **表情**:首次把"困惑"(08-confused)和"满足"(11-satisfied)加入"已用"池 — 003 用 1 deadpan + 1 confused + 1 satisfied 3 种
- **姿态**:新姿态:站+看终端 / 切回敲键盘 / 站凝视空桌面
- **物件**:新物件:终端 / 显示器 / 代码片段 / 散落便签 / 屏幕
- **装扮**:故意 0/4 用, 强调"我 vs AI"的人物关系, 不需要道具干扰

### 4. 文件变更(7 个)

- `doc/essays/003-llm-code-control.md`(新)
- `doc/essays/images/003-llm-code-control/00-stranger-code.png`(新,去 IP)
- `doc/essays/images/003-llm-code-control/01-staring-terminal.png`(新,IP deadpan)
- `doc/essays/images/003-llm-code-control/02-stray-snippets.png`(新,去 IP)
- `doc/essays/images/003-llm-code-control/03-typing-back.png`(新,IP confused)
- `doc/essays/images/003-llm-code-control/04-looking-back.png`(新,IP satisfied)
- `xiaofan-ink/assets/prompts/articles/003-llm-code-control.md`(新,实战 prompt 文档)
- `doc/SERIES-STATE.md`(加 003 行, 更新已用姿态/表情/物件, 加 003 跟 001/002 配对说明)
- `doc/essays/README.md`(待同步:加 003, 跟 001/002 配对, 工具三部曲说明)
- `CHANGELOG.md`(本条 v1.9)

### 5. 用户后续

- 公众号后台 → 草稿箱 → 删 v1.8 / 之前所有 003 的草稿(目前没 003 草稿, 无需删)→ 留 v1.9 这次的 003 新草稿
- 跟 000/001/002 的发布节奏一致:**想写就写, 不定时间**
- 002 17:37 定时发表, 003 待用户决定发布时机

### 6. 实战复盘要点(待生成后补 / 已在 prompt 文档草稿)

- 5 张图全部一次过, 跟 001/002 节奏一致
- 跟 002 的 "工具站错位" 句式区别:003 改成 "番茄钟放在'开始专注'那个位置, 不对, 应该是'做完一件事就停'" — 避免复读 002 原文
- 收尾冷幽默:让 Claude Code 写最后一段注释 — 注释这种"过程"AI 替代得了, 代码本身不行 — 跟前文"过程比结论值钱" 形成首尾呼应
